...
public class CustomerFSM : MonoBehaviour
{
    ...
    private static readonly object priorityLock = new object();     //우선 순위 할달을 위한 동기화 객체
    ...
    void WalkingToShelf()
    {
        if (timer.IsFinisihed() && isMoveDone)
        {
    ...
    void PickingItem()
    {
        if (timer.IsFinisihed())
        {
            if(boxesPicked < boxesToPick)
            {
                ...
                box.transform.localPosition = new Vector3(0f, boxesPicked * 2f, 0f);
